  ...
  admin: {
    ...
    accounts: {
      ...
      codingPlan: {
        ...
        providerHint: 'Kimi、智谱、MiniMax 可尝试内置配额查询；火山与 MiMo 为实验性配额探测；DeepSeek 和自定义兼容供应商依靠 429/401/5xx 被动恢复。',
        ...
        mimo: '小米 MiMo',
        domesticTab: '国内模型',
        dualAccountHint: '只需输入一次 API Key 与 Base URL。系统会按供应商真实能力创建 Chat/Codex（承接 Codex /v1/responses）或 Claude Code（承接 /v1/messages）账号。',
        dualAccountCreated: '已创建 Chat/Codex 与 Claude Code 两个账号。',
        chatOnlyCreated: '已创建 Chat/Codex 账号；该供应商暂不支持 Anthropic Messages，未创建 Claude Code 账号。',
        anthropicSameBaseNote: '该供应商无法自动推导 Anthropic endpoint，不会默认复用 Chat 基础地址。',
        anthropicExplicitOnlyNote: '该供应商不会默认复用 Chat 基础地址创建 Anthropic 账号；只有填写 Anthropic 专用地址才会创建 Claude Code 账号。',
        anthropicOnlyNote: '该入口只创建 Anthropic Messages 账号，用于 Claude Code /v1/messages。',
        chatOnlyNote: '该供应商不支持 Anthropic Messages，当前仅创建 Codex/Chat 账号。',
        baseUrlHint: '供应商基础地址。Kimi/智谱可按预设推导 Anthropic 地址；MiniMax/MiMo 必须显式填写 Anthropic 专用地址。',
        anthropicBaseUrl: 'Anthropic 专用基础地址（可选）',
        anthropicBaseUrlPlaceholder: '例如：https://example.com/anthropic',
        anthropicBaseUrlHint: '仅当该供应商真的提供 Anthropic Messages endpoint 时填写；留空不会创建 Claude Code 账号。',
        baseUrlRequiredPlaceholder: '必填：请输入该供应商的上游基础地址',
        apiFormat: '接口格式',
        apiFormatHint: 'Chat Completions 用于 OpenAI 兼容接口；Anthropic Messages 只承接 Claude Code 风格 /v1/messages。',
        quotaBaseUrl: '实验配额查询基础地址（只读）',
        quotaBaseUrlPlaceholder: '未设置自定义实验探测地址',
        quotaBaseUrlHint: '仅用于展示旧账号或调试账号已有的实验探测地址；前端不再新增或修改该字段。',
        baseUrlRequired: '请填写该平台的上游基础地址。',
        volcengineOpenAPI: '火山 OpenAPI 配额查询',
        volcengineOpenAPIHint: '模型调用继续使用 ark API Key；配额查询需要火山 Access Key ID / Secret Access Key。AK/SK 留空时账号仍可模型调用，但不能自动查询真实套餐额度。',
        volcenginePlanType: '套餐类型',
        volcengineCodingPlan: 'Coding Plan（编程套餐）',
        volcengineAgentPlan: 'Agent Plan（智能体套餐）',
        volcengineRegion: 'Region（地域）',
        volcengineAccessKeyId: 'Access Key ID（访问密钥 ID）',
        volcengineSecretAccessKey: 'Secret Access Key（访问密钥私钥）',
        volcengineSeatId: 'Seat ID（席位 ID）',
        volcengineAccountId: 'Account ID（账号 ID，可选）',
        volcengineProjectName: 'Project Name（项目名，可选）',
        experimentalProbeUrl: '实验性配额探针 URL',
        experimentalProbeUrlHint: '只有确认接口真实可用时再填写；留空时仍标记为实验性配额探测。',
