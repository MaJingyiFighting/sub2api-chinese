...
describe('CreateAccountModal domestic Coding Plan UI', () => {
  ...
  it('creates one dual-protocol account from a single key (kimi)', async () => {
    const wrapper = mountModal()
    const domesticTab = platformTabButtons(wrapper).find((b) => b.text().includes('domesticTab'))
    await domesticTab!.trigger('click')
    await wrapper.vm.$nextTick()
    ...
    expect(payload.credentials.wire_api).toBe('dual')
    expect(payload.credentials.base_url).toBe('https://api.kimi.com/coding/v1')
    expect(payload.credentials.anthropic_base_url).toBe('https://api.kimi.com/coding')
  })
  ...
  it('creates only the Chat/Codex account for volcengine', async () => {
    ...
    expect(createAccountMock).toHaveBeenCalledTimes(1)
    expect(createAccountMock.mock.calls[0][0].name).not.toContain('- Chat/Codex')
  })
})
