...
func (s *AccountTestService) TestAccountConnection(c *gin.Context, accountID int64, modelID string, prompt string, mode string) error {
	...
	if IsCodingPlanChatCompletionsAccount(account) {
		return s.testCodingPlanChatCompletionsAccountConnection(c, account, modelID, prompt)
	}
...
func (s *AccountTestService) testCodingPlanChatCompletionsAccountConnection(c *gin.Context, account *Account, modelID string, prompt string) error {
	...
	authToken := accountCodingPlanAPIKey(account)
	if authToken == "" {
		return s.sendErrorAndEnd(c, "No API key available")
	}
	...
	apiURL := strings.TrimRight(normalizedBaseURL, "/") + "/chat/completions"
	...
	s.sendEvent(c, TestEvent{Type: "test_start", Model: testModelID})
	s.sendEvent(c, TestEvent{Type: "status", Text: "正在通过 Coding Plan /chat/completions 测试连接"})

	ctx := c.Request.Context()
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, apiURL, bytes.NewReader(payloadBytes))
	if err != nil {
		return s.sendErrorAndEnd(c, "Failed to create Coding Plan Chat Completions request")
	}
	req = req.WithContext(WithHTTPUpstreamProfile(req.Context(), HTTPUpstreamProfileOpenAI))
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("Accept", "text/event-stream")
	...
	resp, err := s.httpUpstream.DoWithTLS(req, proxyURL, account.ID, account.Concurrency, s.tlsFPProfileService.ResolveTLSProfile(account))
	if err != nil {
		return s.sendErrorAndEnd(c, fmt.Sprintf("Coding Plan Chat Completions API (/chat/completions) request failed: %s", err.Error()))
	}
	defer func() { _ = resp.Body.Close() }()
	...
	if resp.StatusCode != http.StatusOK {
		body, _ := io.ReadAll(resp.Body)
		if resp.StatusCode == http.StatusUnauthorized && s.accountRepo != nil {
			errMsg := fmt.Sprintf("Coding Plan Chat Completions authentication failed (401): %s", string(body))
			_ = s.accountRepo.SetError(ctx, account.ID, errMsg)
		}
		return s.sendErrorAndEnd(c, fmt.Sprintf("Coding Plan Chat Completions API (/chat/completions) returned %d: %s", resp.StatusCode, string(body)))
	}
