...
func IsCodingPlanAnthropicMessagesAccount(account *Account) bool {
	...
	if strings.TrimSpace(account.GetCredential("anthropic_base_url")) != "" {
		return true
	}
	format := GetCodingPlanAPIFormat(account)
	wireAPI := strings.TrimSpace(account.GetCredential("wire_api"))
...
func (p *codingPlanHTTPProbe) Probe(ctx context.Context, account *Account) (*CodingPlanQuotaSnapshot, error) {
	...
	case CodingPlanProviderVolcengine, CodingPlanProviderMiMo:
		snapshot := unsupportedCodingPlanQuotaSnapshot(p.Provider(), ErrCodingPlanQuotaProbeUnsupported.Error())
		snapshot.QuotaProbeStatus = CodingPlanProbeStatusUnsupported
