...
func DomesticSchedulingPlatforms() []string {
	return []string{
		...
		string(CodingPlanProviderMiMo),
		PlatformDeepSeek,
		PlatformCustomOpenAICompatible,
		PlatformCustomAnthropicCompatible,
	}
}

func AnthropicCompatibleSchedulingPlatforms() []string {
	return append([]string{PlatformAnthropic, PlatformAntigravity}, DomesticSchedulingPlatforms()...)
}
...
func IsDomesticProviderPlatform(platform string) bool {
	platform = strings.ToLower(strings.TrimSpace(platform))
	return IsCodingPlanPlatform(platform) || IsPureKeyDomesticPlatform(platform)
}

func AccountSupportsNativeAnthropicMessages(account *Account) bool {
	if account == nil || account.Type != AccountTypeAPIKey {
		return false
	}
	if account.Platform == PlatformAnthropic || account.Platform == PlatformCustomAnthropicCompatible {
		return true
	}
	if IsCodingPlanAccount(account) && IsCodingPlanAnthropicMessagesAccount(account) {
		return true
	}
	return strings.TrimSpace(account.GetCredential("anthropic_base_url")) != ""
}

func AccountSupportsNativeChatCompletions(account *Account) bool {
	if account == nil || account.Type != AccountTypeAPIKey {
		return false
	}
	if account.IsOpenAI() {
	...
	if !IsDomesticProviderPlatform(account.Platform) {
		return false
	}
	return strings.TrimSpace(account.GetCredential("base_url")) != ""
}

func IsAccountCompatibleWithSchedulingPlatform(account *Account, platform string) bool {
	if account == nil {
		return false
	}
	platform = strings.ToLower(strings.TrimSpace(platform))
	...
	case PlatformDomestic:
		return IsDomesticProviderPlatform(account.Platform)
	case PlatformAnthropic:
		return account.Platform == PlatformAnthropic || AccountSupportsNativeAnthropicMessages(account)
	default:
		return strings.EqualFold(strings.TrimSpace(account.Platform), platform)
	}
}

func RequiresExternalResponsesRouter(platform string) bool {
